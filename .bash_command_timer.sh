...
function BCTPostCommand() {
  ...
  [ $EXIT_CODE -eq 0 ] || local DIV="| $EXIT_CODE"
  [ $EXIT_CODE -eq 0 ] || local DIV_C="| \033[31m$EXIT_CODE\033[${BCT_COLOR}m"
  if [ -n "$now_str" ]; then
    local output_str="[ $time_str $DIV | $now_str ]"
    local output_str1="[ $time_str $DIV_C | $now_str ]"
  else
    local output_str="[ $time_str $DIV ]"
    local output_str1="[ $time_str $DIV_C ]"
  fi
  
  if [ -n "$BCT_COLOR" ]; then
    local output_str_colored="\033[${BCT_COLOR}m${output_str1}\033[0m"
  else
